...
func (h *LogisticsHandler) GetTrack(w http.ResponseWriter, r *http.Request) {
	...
	if fromStr == "" || toStr == "" {
		respondError(w, errs.NewBadRequest("from and to query parameters are required"))
		return
	}

	from, err := time.Parse(time.RFC3339, fromStr)
	if err != nil {
		respondError(w, errs.NewBadRequest("invalid from time format, use RFC3339"))
		return
	}
	to, err := time.Parse(time.RFC3339, toStr)
	if err != nil {
		respondError(w, errs.NewBadRequest("invalid to time format, use RFC3339"))
		return
	}
