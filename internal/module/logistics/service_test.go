...
func (m *mockLogisticsRepo) GetRoute(_ context.Context, wsID, routeID uuid.UUID) (*Route, error) {
	r, ok := m.routes[routeID]
	if !ok || r.WorkspaceID != wsID {
...
func (m *mockLogisticsRepo) ListRoutes(_ context.Context, wsID uuid.UUID, filter RouteFilter) ([]Route, int, error) {
	var result []Route
...
func (m *mockLogisticsRepo) GetStop(_ context.Context, wsID, stopID uuid.UUID) (*RouteStop, error) {
	s, ok := m.stops[stopID]
...
func TestCreateRoute(t *testing.T) {
	...
	route, err := svc.CreateRoute(context.Background(), wsID, input, &actorID)
	require.NoError(t, err)
	assert.Equal(t, "Route A", route.Name)
	assert.Equal(t, "planned", route.Status)
	assert.Equal(t, wsID, route.WorkspaceID)
...
func TestGetRouteWithStops(t *testing.T) {
	...
	route := &Route{ID: uuid.New(), WorkspaceID: wsID, Name: "R1", Status: "planned"}
	repo.routes[route.ID] = route

	stop := &RouteStop{ID: uuid.New(), RouteID: route.ID, WorkspaceID: wsID, Address: "Addr", Status: "pending"}
	repo.stops[stop.ID] = stop

	result, err := svc.GetRoute(context.Background(), wsID, route.ID)
	require.NoError(t, err)
...
func TestStartRoute(t *testing.T) {
	...
	result, err := svc.StartRoute(context.Background(), wsID, route.ID, &actorID)
	require.NoError(t, err)
...
func TestStartRouteWrongStatus(t *testing.T) {
	...
	route := &Route{ID: uuid.New(), WorkspaceID: wsID, Name: "R1", Status: "in_progress"}
	repo.routes[route.ID] = route

	_, err := svc.StartRoute(context.Background(), wsID, route.ID, nil)
	require.Error(t, err)
...
func TestCompleteRoute(t *testing.T) {
	...
	now := time.Now()
	route := &Route{ID: uuid.New(), WorkspaceID: wsID, Name: "R1", Status: "in_progress", ActualStart: &now}
	repo.routes[route.ID] = route

	result, err := svc.CompleteRoute(context.Background(), wsID, route.ID, &actorID)
	require.NoError(t, err)
...
func TestCompleteRouteWrongStatus(t *testing.T) {
	...
	_, err := svc.CompleteRoute(context.Background(), wsID, route.ID, nil)
	require.Error(t, err)
...
func TestArriveAtStop(t *testing.T) {
	...
	stop := &RouteStop{ID: uuid.New(), RouteID: routeID, WorkspaceID: wsID, Status: "pending"}
	repo.stops[stop.ID] = stop

	result, err := svc.ArriveAtStop(context.Background(), wsID, routeID, stop.ID, &actorID)
	require.NoError(t, err)
...
func TestArriveAtStopWrongRoute(t *testing.T) {
	...
	stop := &RouteStop{ID: uuid.New(), RouteID: uuid.New(), WorkspaceID: wsID, Status: "pending"}
	repo.stops[stop.ID] = stop

	_, err := svc.ArriveAtStop(context.Background(), wsID, uuid.New(), stop.ID, nil)
	require.Error(t, err)
...
func TestArriveAtStopAlreadyVisited(t *testing.T) {
	...
	stop := &RouteStop{ID: uuid.New(), RouteID: routeID, WorkspaceID: wsID, Status: "arrived"}
	repo.stops[stop.ID] = stop

	_, err := svc.ArriveAtStop(context.Background(), wsID, routeID, stop.ID, nil)
	require.Error(t, err)
...
func TestCompleteStop(t *testing.T) {
	...
	stop := &RouteStop{ID: uuid.New(), RouteID: routeID, WorkspaceID: wsID, Status: "arrived", DeliveryIDs: deliveryIDs}
	repo.stops[stop.ID] = stop

	result, err := svc.CompleteStop(context.Background(), wsID, routeID, stop.ID, &actorID)
	require.NoError(t, err)
	assert.Equal(t, "completed", result.Status)
...
func TestCompleteStopMustBeArrived(t *testing.T) {
	...
	_, err := svc.CompleteStop(context.Background(), wsID, routeID, stop.ID, nil)
	require.Error(t, err)
...
func TestUpdateGeo(t *testing.T) {
	...
	err := svc.UpdateGeo(context.Background(), wsID, entityID, point, &actorID)
	require.NoError(t, err)
...
func TestGetTrack(t *testing.T) {
	...
	points, err := svc.GetTrack(context.Background(), wsID, entityID, now.Add(-3*time.Hour), now)
	require.NoError(t, err)
...
func TestFullRouteLifecycle(t *testing.T) {
	...
	route, err := svc.CreateRoute(context.Background(), wsID, CreateRouteInput{
		Name: "Delivery Run",
		Stops: []CreateStopInput{
	...
	// Start
	route, err = svc.StartRoute(context.Background(), wsID, route.ID, &actorID)
	require.NoError(t, err)
	assert.Equal(t, "in_progress", route.Status)

	// Arrive at first stop
	stop1, err := svc.ArriveAtStop(context.Background(), wsID, route.ID, stopIDs[0], &actorID)
	require.NoError(t, err)
	assert.Equal(t, "arrived", stop1.Status)

	// Complete first stop
	stop1, err = svc.CompleteStop(context.Background(), wsID, route.ID, stopIDs[0], &actorID)
	require.NoError(t, err)
	assert.Equal(t, "completed", stop1.Status)

	// Complete route
	route, err = svc.CompleteRoute(context.Background(), wsID, route.ID, &actorID)
	require.NoError(t, err)
